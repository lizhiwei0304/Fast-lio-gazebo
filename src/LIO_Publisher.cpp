...
LIO_Pub::LIO_Pub(ros::NodeHandle &n)
{
  ...
  n.param<string>("tp_sub_pcd", tp_sub_pcd, "cloud_registered");
  n.param<string>("tp_sub_odom", tp_sub_odom, "Odometry");
  ...
  odometrySubscriber_ = n.subscribe(NameSpace + "/" + tp_sub_odom, 1, &LIO_Pub::OdomCallback, this);
  pointCloudSubscriber_ = n.subscribe(NameSpace + "/" + tp_sub_pcd, 1, &LIO_Pub::PCCallback, this);
...
void LIO_Pub::OdomCallback(nav_msgs::Odometry msg)
{
  ...
  if (distance > dis_th)
  {
    ...
    {
      tmpNameSpace = NameSpace.substr(1); // 从索引 1 开始截取
    }
...
void LIO_Pub::pub_TF(nav_msgs::Odometry msg)
{
  ...
  tf_broadcaster.sendTransform(tf::StampedTransform(transform, msg.header.stamp, tmpNameSpace + "/odom", tmpNameSpace + SensorName));
}
