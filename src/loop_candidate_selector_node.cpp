...
private:
    ...
    void submapCallback(const dislam_msgs::SubMapConstPtr &msg)
    {
        ...
        const std::vector<KeyframeCloudScorer::Score> scores = scorer_.evaluateAll(keyframes_);
        const std::vector<int> selected = scorer_.selectCandidates(keyframes_, scores, nms_distance_, max_candidate_num_);
