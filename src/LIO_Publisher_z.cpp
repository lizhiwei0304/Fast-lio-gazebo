...
private:
  ...
  ros::Publisher modifyOdomPublisher_;
...
LIO_Pub::LIO_Pub(ros::NodeHandle &n)
{
  ...
  n.getParam("UseGPSAltitute", UseGPSAltitute);
  ...
  gpsSubscriber_ = n.subscribe(NameSpace + "/gps", 1, &LIO_Pub::GPSCallback, this);
  ...
  modifyOdomPublisher_ = n.advertise<nav_msgs::Odometry>(NameSpace + "/modify_odom", 5);
}
...
void LIO_Pub::GPSCallback(const sensor_msgs::NavSatFix::ConstPtr &msg)
{
  if (isFirstGPS)
  {
  ...
  // std::cout << "altitude in callback = " << altitude << std::endl;
}
